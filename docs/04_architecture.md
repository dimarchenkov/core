...
Каждый бизнес-модуль внутри `backend/app/` должен иметь похожую структуру:
...
Текущий остаток хранится отдельно для быстрого чтения, но объясняется историей движений.
