...
| `CompleteIntakeWorkflow` | Intake | Application / Workflow Service | Idempotent atomic materialization into Catalog, optional Pricing, Receipt, Inventory and Readiness | transaction-neutral services/repositories across contexts | Cohesive workflow and sole transaction owner for Complete Intake |
| `RentalLifecycleService` | Rental | Application / Read Service | Append maintenance, damage and condition-photo evidence; derive asset passports and customer rental history | Rental projections, Catalog, Customers, Media and Identity | Journals remain outside RentalAsset/RentalOrder aggregates; checkout and return workflows are unchanged |
| `RentalEconomicsService` | Rental | Read Service | Derive revenue, expenses, utilization, payback and aggregate economics without persisted analytical state | RentalAsset, completed RentalOrderItem and maintenance facts; Catalog hierarchy | Rules are computed on request; no Aggregate or workflow mutation |
...
| `ActivityEventService` | Activity | Domain Service | Append the five supported Intake outcome facts to the caller-owned transaction | `ActivityEventRepository` | Specialized methods deliberately prevent arbitrary event/payload creation |
| `ActivityReadService` | Activity | Read Service | Current employee activity feed with type filtering and pagination | `ActivityEventRepository` | Owner-wide views and aggregation are intentionally deferred |
