...
- `RentalOrder` сохраняет оператора выдачи в `issued_by`.
- `RentalOrder` сохраняет оператора завершения возврата в `returned_by`.
- При частичном возврате каждая позиция дополнительно сохраняет собственного оператора возврата.
