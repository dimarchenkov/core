...
Asset отвечает на вопрос: «Какой именно экземпляр выдан клиенту?»

Asset имеет собственный код, состояние, комплектность, пломбы, осмотры, обслуживание и историю выдач. Он не заменяет Product или Variant.
...
Жизненный цикл временной передачи конкретного Asset клиенту.
...
Asset          → идентифицирует арендный экземпляр
Rental         → управляет временной выдачей Asset
